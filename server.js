...
app.post('/api/paymentverification', async (req, res) => {
  ...
  if (expectedSignature === razorpay_signature) {
    // Save payment details to DB (assuming Payment model exists)
...
app.post('/forgotpassword', async (req, res) => {
  ...
  const resetToken = crypto.randomBytes(32).toString('hex');
  resetTokens[email] = { token: resetToken, expiry: Date.now() + 15 * 60 * 1000 };
...
app.post('/login', async (req, res) => {
  ...
  const user = await User.findOne({ email });
  if (!user) return res.status(400).json({ error: "User not found" });
  if (!user.verifiedUser) return res.status(403).json({ error: "Please verify your email" });
...
app.post('/registerevent', async (req, res) => {
  ...
  try {
    const existingRegistration = await EventRegistration.findOne({ teamLeaderEmail, eventName });
    if (existingRegistration) return res.status(400).json({ error: 'Registration already exists for this event.' });

    const registration = new EventRegistration({ eventName, teamLeaderName, teamLeaderMobileNo, teamLeaderEmail, teamLeaderCollege, teamSize, teamLeaderGender, fees });
    await registration.save();
    res.status(200).json({ success: true, message: 'Successfully added to cart! Pay to complete registration process.' });
  } catch (error) {
    console.error('Database save error:', error);
